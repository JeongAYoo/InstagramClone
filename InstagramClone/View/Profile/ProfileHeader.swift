...
class ProfileHeader: UICollectionReusableView {
    ...
    private let profileImageView: UIImageView = {
        let iv = UIImageView()
        iv.image = UIImage(named: "markFeed6")
    ...
    override init(frame: CGRect) {
        ...
        profileImageView.layer.cornerRadius = 80 / 2    // give it circular shape
        ...
        addSubview(stack)
        stack.centerY(inView: profileImageView)     // 프로필이미지와 centerY 동일하게
        stack.anchor(left: profileImageView.rightAnchor, right: rightAnchor,
                     paddingLeft: 12, paddingRight: 12, height: 50)
        
        // Edit Profile 버튼 아래 Tool bar
        let topDivider = UIView()        // 얇은 회색선으로 보이는 divider 만들기
